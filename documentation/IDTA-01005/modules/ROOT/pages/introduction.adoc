...
This specification is versioned using https://semver.org/spec/v2.0.0.html[Semantic Versioning 2.0.0] (semver) and follows the semver specification xref:bibliography.adoc#semverSemanticVersioning[].
...
Please consult the continuously updated reading guide xref:bibliography.adoc#aasReadingGuide[] for an overview of documents on the Asset Administration Shell. The reading guide gives advice on which documents should be read depending on the role of the reader.
...
The document series "Details of the Asset Administration Shell" specifies the different needs of implementing Asset Administration Shells in an Industry 4.0 environment. A corresponding IEC series is in development, see xref:bibliography.adoc#iec63278-1[].
...
. definition of the Asset Administration Shell in a selected format, for example XML (see xref:bibliography.adoc#idta01001[]),
...
. defining a secure way to exchange the file, for example via secure file download on a web server xref:bibliography.adoc#secureDownloadService[].
...
The exchange of files needs to fulfil some requirements with respect to usability and security xref:bibliography.adoc#secureDownloadService[].A bilateral agreement on security constraints is required, which must be fulfilled for the transfer and usage of the files.Please refer to Part 4 of the series "Details of the Asset Administration Shell" for more details.
...
The *physical model* maps these logical concepts to a physical format. The result of this mapping is a physical package format (a ZIP archive format) in which files appear in a directory-like hierarchy (adapted from xref:bibliography.adoc#microsoftDigitalSigning[] and xref:bibliography.adoc#microsoftOpenPackaging[]).
